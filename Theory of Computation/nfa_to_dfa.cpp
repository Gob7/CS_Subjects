...
int main()
{
    ...
    int cur_sta[nfa_sta_num], cur_sta_len = 1;
    cur_sta[0] = 0;
    ...
    while (1)
    {
        ...
        if (flag1)
        {
            ...
            for (i = 0; i < cur_sta_len; i++)
                dfa_sta[dfa_sta_cnt][i] = cur_sta[i];
            ...
            for (i = 0; i < nfa_fin_cnt; i++)
            {
                for (j = 0; j < cur_sta_len; j++)
                {
                    if (nfa_fin[i] == cur_sta[j])
                    {
                        ...
                        for (k = 0; k < cur_sta_len; k++)
                            dfa_fin[dfa_fin_cnt][k] = cur_sta[k];
                        dfa_fin_cnt++;
                        flag2 = 1;
                        break;
                    }
        ...
        if (dfa_sta_cnt * inp == (p1 + 1) * (p2 + 1) && !flag1)
            break;
        p2++;
        if (p2 == inp)
        {
    ...
    for (i = -1; i < dfa_sta_cnt; i++)
    {
        for (j = -1; j < inp; j++)
        {
            ...
            if (j == -1)
            {
                for (k = 0; k < dfa_sta_len[i]; k++)
                    cout << dfa_sta[i][k];
                continue;
            }
            cout << "\t";
            for (k = 0; k < dfa_len[i][j]; k++)
                cout << dfa[i][j][k];
        }
        cout << endl;
    }
