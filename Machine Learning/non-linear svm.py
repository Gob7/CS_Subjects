...
def tf(p):
    ...
    for i, d in enumerate(dt):
        if d>2:
            p[i] = [4-p[i][1]+abs(p[i][0]-p[i][1]), 4-p[i][0]+abs(p[i][0]-p[i][1])]
    return p
...
if w[1] != 0:
    ...
    plt.axvline(x=-bias/(w[0]+eps), color='black', linestyle='-', label='Hyperplane')
plt.scatter(support_vectors[:, 0], support_vectors[:, 1], color='grey', marker='s', label='Support Vectors')
plt.xlabel('X1')
plt.ylabel('X2')
